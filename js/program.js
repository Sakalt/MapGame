...
function start() {
    ...
    if(nth_question <= 50) {
        ...
        /*
        if(nth_question == 1) {country_code = numArr.slice(0, 1);}
        if(nth_question == 2) {country_code = numArr.slice(1, 2);}
        if(nth_question == 3) {country_code = numArr.slice(2, 3);}
        if(nth_question == 4) {country_code = numArr.slice(3, 4);}
        if(nth_question == 5) {country_code = numArr.slice(4, 5);}
        if(nth_question == 6) {country_code = numArr.slice(5, 6);}
        if(nth_question == 7) {country_code = numArr.slice(6, 7);}
        if(nth_question == 8) {country_code = numArr.slice(7, 8);}
        if(nth_question == 9) {country_code = numArr.slice(8, 9);}
        if(nth_question == 10) {country_code = numArr.slice(9, 10);}
        if(nth_question == 11) {country_code = numArr.slice(10, 11);}
        if(nth_question == 12) {country_code = numArr.slice(11, 12);}
        if(nth_question == 13) {country_code = numArr.slice(12, 13);}
        if(nth_question == 14) {country_code = numArr.slice(13, 14);}
        if(nth_question == 15) {country_code = numArr.slice(14, 15);}
        if(nth_question == 16) {country_code = numArr.slice(15, 16);}
        if(nth_question == 17) {country_code = numArr.slice(16, 17);}
        if(nth_question == 18) {country_code = numArr.slice(17, 18);}
        if(nth_question == 19) {country_code = numArr.slice(18, 19);}
        if(nth_question == 20) {country_code = numArr.slice(19, 20);}
        if(nth_question == 21) {country_code = numArr.slice(20, 21);}
        if(nth_question == 22) {country_code = numArr.slice(21, 22);}
        if(nth_question == 23) {country_code = numArr.slice(22, 23);}
        if(nth_question == 24) {country_code = numArr.slice(23, 24);}
        if(nth_question == 25) {country_code = numArr.slice(24, 25);}
        if(nth_question == 26) {country_code = numArr.slice(25, 26);}
        if(nth_question == 27) {country_code = numArr.slice(26, 27);}
        if(nth_question == 28) {country_code = numArr.slice(27, 28);}
        if(nth_question == 29) {country_code = numArr.slice(28, 29);}
        if(nth_question == 30) {country_code = numArr.slice(29, 30);}
        if(nth_question == 31) {country_code = numArr.slice(30, 31);}
        if(nth_question == 32) {country_code = numArr.slice(31, 32);}
        if(nth_question == 33) {country_code = numArr.slice(32, 33);}
        if(nth_question == 34) {country_code = numArr.slice(33, 34);}
        if(nth_question == 35) {country_code = numArr.slice(34, 35);}
        if(nth_question == 36) {country_code = numArr.slice(35, 36);}
        if(nth_question == 37) {country_code = numArr.slice(36, 37);}
        if(nth_question == 38) {country_code = numArr.slice(37, 38);}
        if(nth_question == 39) {country_code = numArr.slice(38, 39);}
        if(nth_question == 40) {country_code = numArr.slice(39, 40);}
        if(nth_question == 41) {country_code = numArr.slice(40, 41);}
        if(nth_question == 42) {country_code = numArr.slice(41, 42);}
        if(nth_question == 43) {country_code = numArr.slice(42, 43);}
        if(nth_question == 44) {country_code = numArr.slice(43, 44);}
        if(nth_question == 45) {country_code = numArr.slice(44, 45);}
        if(nth_question == 46) {country_code = numArr.slice(45, 46);}
        if(nth_question == 47) {country_code = numArr.slice(46, 47);}
        if(nth_question == 48) {country_code = numArr.slice(47, 48);}
        if(nth_question == 49) {country_code = numArr.slice(48, 49);}
        if(nth_question == 50) {country_code = numArr.slice(49, 50);}
        */
       /*
        var test222 = nth_question - 1;
        let country_code = numArr[test222];
        */
        console.log(nth_question);
        var country_code = Number(nth_question);
        ...
        //地図画像の幅と高さを変数に代入
        map_width = document.getElementById('svg808').clientWidth;
        map_height = document.getElementById('svg808').clientHeight;
        //基準値
        base_x = 0.039419425;
        base_y = 0.07;
        console.log(country_code);
...
function answer_button_click(){
    ...
    if(document.getElementById("country_name_input_field").value == ''){
        ...
        //↓変数country_codeの桁数が1桁なら、始めに空欄を追加する(見やすくするため)           ★シングルクオーテーション固定★
        if(parseInt(country_code).toString(10).length == 1){country_code = "<div class='mgr-05rem'></div>" + country_code;}
        //↓わからない・不正解問題の国名の配列に正しい国名を挿入(<br>タグもいれてる)
        incorrect_answer_country.unshift(country_code+"￤"+answer_country_name+"<br>");
        //↓次の問題を出す
        start();
    }else{
        ...
        for (let i = 0; i < arraySplit.length; i++) {
            ...
            if(String(cc).match(/^([\u{3005}\u{3007}\u{303b}\u{3400}-\u{9FFF}\u{F900}-\u{FAFF}\u{20000}-\u{2FFFF}][\u{E0100}-\u{E01EF}\u{FE00}-\u{FE02}]?)+$/mu) || String(cc).match(/^[\u{3000}-\u{301C}\u{30A1}-\u{30F6}\u{30FB}-\u{30FE}]+$/mu)){
                console.log((i+1) + "文字目は漢字またはカタカナです。");
            }else{
                swal_boot = true;
            }
        }
        ...
        if(swal_boot == false){
            ...
            an3.style.display ="none";
                ...
                console.log(typeof answer_field);
                console.log(typeof answer_country_name);
                console.log(answer_country_name);
                if(answer_field == answer_country_name){
                    ...
                    //↓効果音チェックボタンがチェックされてるなら、効果音を鳴らす
                    if(sound_effect_button.checked){document.getElementById('incorrect').play();}
                    //↓変数country_codeの桁数が1桁なら、始めにaを追加する(見栄えをよくするため)           ★シングルクオーテーション固定★
                    if(parseInt(country_code).toString(10).length == 1) {country_code = "<div class='mgr-05rem'></div>" + country_code;}
                    //↓わからない・不正解問題の国名の配列に正しい国名を挿入(<br>タグもいれてる)
                    incorrect_answer_country.unshift(country_code+"￤"+answer_country_name+"<br>");
                    //↓次の問題を出す
...
function country_list(){
    ...
    if(country_list_display == true){
        if(countrylist.style.display=="block"){
            ...
            //document.getElementById('country_listbutton').innerHTML = '<i class="fas fa-chevron-down"></i>出題国リスト';
        }else{
            countrylist.style.display ="block";
            country.style.display ="none";
            document.getElementById("country_list_arrow").className = "fas fa-chevron-up";
            //document.getElementById('country_listbutton').innerHTML = '<i class="fas fa-chevron-up"></i>出題国リスト';
        }
    }else{
        countrylist.style.display ="none";
        //document.getElementById("country_list_arrow").className = "fas fa-chevron-down";
